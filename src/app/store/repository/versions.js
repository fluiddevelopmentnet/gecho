...
export function createVersionFile(repo_id, new_version_id, old_file_path, contributor){
    return fs.writeFileSync(`${root}/${repo_id}/history/${new_version_id}.version.json`,JSON.stringify({
        contributor: contributor,
        changes: getDiff(new_version_id, old_file_path)
    }))
}
...
export function createInitialVersionFile(repo_id, new_version_id, old_file_path, contributor){
    return fs.writeFileSync(`${root}/${repo_id}/history/${new_version_id}.version.json`,JSON.stringify({
        contributor: contributor,
...
export function getVersionChangeChain(repo_id, version_id){
    let result = []
    ...
    function getVersionChanges(rec_version_id){
        ...
        if (version_object["merged"]===true|| version_object["init"]===true) {
